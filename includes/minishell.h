...
/*   Updated: 2025/07/24 17:51:34 by lfournie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
# define SHL_NAME_ERR "toupetishell"
...
void		ft_fill_env(t_minishell *shl, t_env **env_list, char **env);
t_env		*ft_lstlast_env(t_env *lst);
void		ft_lstadd_back_env(t_env **lst, t_env *new);
t_env		*ft_lstnew_env(void *content);
void		ft_print_env(t_env *env);
void		ft_handle_shlvl(t_minishell *shl, t_env *env);
void		ft_handle_shlvl_in_array(t_minishell *shl, char **env);
int			ft_export_forempty_env(t_minishell *shl);	
int			ft_fork_heredoc(t_minishell *shl, char *limiter, int index);
char		*ft_name_the_heredoc_file(t_minishell *shl, int index);
void		ft_handle_heredoc(t_minishell *shl, char *limiter, int index);
int			ft_wait_heredoc_child(t_minishell *shl, pid_t pid);
char		*ft_strjoin_heredoc(char *s1, char *s2);
t_redirect	*ft_lstnew_redirect(void *content, int type);
t_redirect	*ft_lstnew_redirect_heredoc(t_minishell *shl,
				char *heredoc_file_name);
t_redirect	*ft_lstlast_redirect(t_redirect *lst);
void		ft_execve(t_minishell *shl, int index, char *cmd);
void		ft_lstadd_back_redirect(t_redirect **lst, t_redirect *new);
int			ft_execution(t_minishell *shl);
int			ft_init_struct_foreach_cmd(t_minishell *shl);
int			ft_fill_redirection(t_minishell *shl, int i_struct, t_token *temp);
void		ft_put_input_to_struct(t_minishell *shl,
				int i_struct, t_token *temp);
void		ft_put_output_to_struct(t_minishell *shl,
				int i_struct, t_token *temp);
void		ft_put_append_to_struct(t_minishell *shl,
				int i_struct, t_token *temp);
void		ft_put_heredoc_to_struct(t_minishell *shl,
				int index, char *heredoc_file_name);
int			ft_put_word_to_struct(t_minishell *shl,
				int i_struct, int i_args, t_token *temp);
void		ft_fill_heredoc_history(t_minishell *shl, int index);
void		ft_get_nb_of_cmd(t_minishell *shl);
void		ft_total_exit(t_minishell *shl);
void		ft_error_msg(t_minishell *shl,
				char *shell_name, char *cmd, char *msg);
void		ft_get_nb_of_words(t_minishell *shl);
void		ft_save_std_fileno(t_minishell *shl);
void		ft_change_pwd(t_minishell *shl, t_env *env, char *directory);
bool		ft_is_option(char *args);
char		*ft_get_home_path(t_env *env);
void		ft_if_child(t_minishell *shl, int index, pid_t pid);
void		ft_export_path_forempty_env(t_minishell *shl, char *pwd);
void		ft_check_signals(t_minishell *shl, char *lim_tmp, char *line);
char		*ft_name_the_heredoc_file(t_minishell *shl, int index);
void		ft_export_shlvl_forempty_env(t_minishell *shl, char *pwd);
void		ft_check_close_dup(t_minishell *shl, int index);
int			ft_just_export(t_env *env);
int			ft_export_value(t_minishell *shl, int index);
int			ft_option_check(t_minishell *shl, int index, int i);
...
char		*ft_find_absolute_path(t_minishell *shl, int index);
int			ft_parent_process(t_minishell *shl);
int			ft_creating_child(t_minishell *shl, int index, pid_t pid);
int			ft_execute_one_cmd(t_minishell *shl, char *cmd, int index);
bool		ft_exec_built_in_cmd(t_minishell *shl, int index, char *cmd);
void		ft_cmd_checking(t_minishell *shl, int index, char *cmd);
int			ft_simple_cmd(t_minishell *shl, int index);
void		ft_simple_cmd_withpipe(t_minishell *shl, int index);
void		ft_child_loop(t_minishell *shl, int index);
void		ft_waiting_for_child(t_minishell *shl,
				int index, int nb, pid_t pid);
int			ft_copy_env_for_execve(t_minishell *shl);
int			ft_malloc_env_for_execve(char ***env, int nb);
int			ft_count_var_in_env(t_env *env);
int			ft_redirections(t_minishell *shl, int index);
int			ft_redir_input(t_minishell *shl, int index);
int			ft_redir_output(t_minishell *shl, int index);
int			ft_check_infile(t_minishell *shl, char *input);
int			ft_env(t_minishell *shl, int index);
int			ft_echo(t_minishell *shl, int index);
int			ft_cd(t_minishell *shl, int index);
int			ft_pwd(t_minishell *shl, int index);
int			ft_export(t_minishell *shl, int index);
int			ft_unset(t_minishell *shl, int index);
int			ft_exit(t_minishell *shl, int index);
char		*ft_expand_line_heredoc(t_minishell *shl, char *expand_me);
char		*ft_quotes(char *has_quotes, char *result);
bool		ft_quote_or_not_quote(char *limiter);
char		*ft_unquote_limiter(char *unquote_me);
char		*ft_unquote_lim_heredoc(t_minishell *shl, char *limiter);
void		ft_write_to_stderr(t_minishell *shl,
				char *msg1, char *arg, char *msg2);
bool		ft_find_oldpwd_in_env(t_minishell *shl);
void		ft_export_oldpwd(t_minishell *shl);
void		ft_syscall_ft_failed(t_minishell *shl, char *cmd);
bool		ft_check_var_name(char *name);
void		ft_unlink_heredoc_files(t_minishell *shl);
void		ft_liberate_the_child(t_minishell *shl);
void		ft_handle_err_msg(t_minishell *shl, int err_code);
